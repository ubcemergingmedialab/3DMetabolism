AFRAME.registerComponent('arrow-head', {
    ...
    update: function(direction, isVisible) {
        ...
        switch(direction) {
            case this.directions.FORWARD:
                ...
                if(showArrow != undefined) { this.changeOpacity(direction,showArrow) }
                this.updateState(isVisible)
                break;
            case this.directions.BACKWARD:
                showArrow = this.checkVisibilityEdgeCase(this.state.BACKWARD,isVisible)
                if(showArrow != undefined) { this.changeOpacity(direction,showArrow) }
                this.updateState(isVisible)
                break;
        }
    ...
    checkVisibilityEdgeCase: function(state,isVisible) {
        var showElem
        if(state === this.states.ENABLED && !!!isVisible) { showElem = false }
        if(state === this.states.DISABLED && isVisible) { showElem = true }
        return showElem
    },

    updateState: function(isVisible) {
        if(this.state.FORWARD - 1 < 0 && !!!isVisible) {return}
        isVisible ? this.state.FORWARD++ : this.state.FORWARD--
    },

    changeOpacity: function(arrowHeadDirection,isVisible) {
        const parent = document.getElementById("arrow-" + this.data.input + "/" + this.data.output)
    ...
    initArrowHeads: function() {
        ...
        arrowParent.setAttribute("id","arrow-" + this.data.input + "/" + this.data.output)
        ...
        let head = (new THREE.Vector3()).copy((this.fetchNode(this.data.output)).object3D.position)
        let tail = (new THREE.Vector3()).copy((this.fetchNode(this.data.input)).object3D.position)
