AFRAME.registerComponent("pathway-focus", {
  ...
  zoomIn: function () {
    const pos =
      this.data.targetPos.x +
      " " +
      this.data.targetPos.y +
      " " +
      this.data.targetPos.z;
  ...
  zoom: function (pos, eventTag) {
    ...
    const networkView = document.getElementById("parent-model").components[
      "network-view"
    ];
  ...
  createZoomOutButton: function () {
    window.setTimeout((e) => {
      ...
      flexContainer.object3D.position.copy(new THREE.Vector3(-7, -2.5, -5));
    }, this.data.animationDur);
  },
});
